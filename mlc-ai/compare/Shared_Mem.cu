...
int main() {
    const int n = 1024*1024; // Increase n to have a larger workload
    float *in1, *in2, *out;
    std::cout<<in1<<"   "<<in2<<"   "<<out<<std::endl;
    cudaMallocManaged(&in1, n * sizeof(float));
    cudaMallocManaged(&in2, n * sizeof(float));
    ...
    int blockSize = 1024; // Define block size
    // int blockSize = 1024; // change this when talking about occupancy
    int numBlocks = 1024; // Ensure there are enough blocks to cover all elements

    // Launch non-coalesced kernel
    main_kernel<<<numBlocks, blockSize>>>(in1, in2, out);
